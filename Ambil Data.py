...
model = load_model(r"2Label.h5", compile=False)
    ...
    try:
        ...
        if pos_frame%1800 == 0:
            ...
            for i in indices:
                ...
                classess = ['Biru','Hijau','Hitam','Merah','Putih'] #Get array of all classes
                ...
                # cv2.putText(img, JenisKendaraan, (x1,y1+h+50),cv2.FONT_HERSHEY_COMPLEX, 0.5,(255,255,255),1)
                    ...
                    if total_sum_strings == 7:
                        cropped_object = image[y1:y1+h, x1:x1+w]
                        original_width, original_height = cropped_object.shape[:2]
                        new_image = np.zeros((300, 310, 3), np.uint8)

                        cropped_object = imutils.resize(cropped_object, height=160)

                        height, width = cropped_object.shape[:2]
                        if width > 310:
                            cropped_object = cv2.resize(cropped_object, (310, 160))
                        height, width = cropped_object.shape[:2]
                        new_image[:height, :width] = cropped_object
                        ...
                        if label == "Kendaraan Berat":
                            ...
                            cv2.putText(new_image, TipeKendaraan, (10, 200), font, font_scale, color, thickness)
                            gambar_crop += 1
                            # Save the final image
                            cv2.imwrite(f'hasil data/{int(pos_frame):04d}_{gambar_crop:04d}.jpg', new_image)
                        else:
                            draw_text(img, TipeKendaraan, x1, y1-50, (0,255,0))
                            draw_text(img, WarnaKendaraan, x1, y1-15, (0,255,0))
                            ...
                            cv2.putText(new_image, TipeKendaraan, (10, 200), font, font_scale, color, thickness)
                            cv2.putText(new_image, WarnaKendaraan, (10, 220), font, font_scale, color, thickness)
                            # cv2.putText(new_image, JenisKendaraan, (10, 240), font, font_scale, color, thickness)
                            # cv2.putText(new_image, klasikmodern, (10, 260), font, font_scale, color, thickness)
                            # cv2.putText(new_image, merkkendaraan, (10, 280), font, font_scale, color, thickness)
                            gambar_crop += 1
                            # Save the final image
                            cv2.imwrite(f'Data 2 Label/Vidio 1/{int(pos_frame):04d}_{gambar_crop:04d}.jpg', new_image)
